...
grep -q "frame 5" build/bfdoom-smoke-scripted.err
...
grep -q "frame 11" build/bfdoom-smoke-sprite.err
test -s build/bfdoom-smoke-sprite.ppm

rm -f build/bfdoom-smoke-rotated-sprite.ppm build/bfdoom-smoke-rotated-sprite.err
printf "aaaaaaaaaaaa" | timeout 10 "$ELVM/out/bfopt" -doom-host -wad "$WAD" \
  -capture build/bfdoom-smoke-rotated-sprite.ppm "$BF" \
  >/dev/null 2>build/bfdoom-smoke-rotated-sprite.err
grep -q "sprite type=3004 frame=A rotation=2 lump=POSSA2A8 flip=0" build/bfdoom-smoke-rotated-sprite.err
grep -q "sprite_anim type=3001 frame=B rotation=2 lump=TROOB2B8 flip=0" build/bfdoom-smoke-rotated-sprite.err
...
grep -q "frame 26" build/bfdoom-smoke-use.err
...
grep -q "frame 3" build/bfdoom-smoke-weapon.err
