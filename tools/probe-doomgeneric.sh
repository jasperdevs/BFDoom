...
mkdir -p "$OUT"
rm -f "$OUT"/*.eir "$OUT"/*.log
: > "$OUT/summary.txt"

cd "$ELVM" || exit 1

for base in "${SOURCES[@]}" doomgeneric_bf; do
  if [ "$base" = "doomgeneric_bf" ]; then
    file="$ROOT/ports/bfdoom/doomgeneric_bf.c"
  else
    file="$DOOM/$base.c"
  fi

  if ./out/8cc -S -D__eir__ -DINT_MIN=-16777216 -DSHRT_MAX=32767 -DEISDIR=21 -DSEEK_SET=0 -DSEEK_END=2 -I"$PORT_LIBC" -I. -Ilibc -Iout -I"$DOOM" -o "$OUT/$base.eir" "$file" > "$OUT/$base.log" 2>&1; then
    printf "OK %s\n" "$base" | tee -a "$OUT/summary.txt"
  else
    printf "FAIL %s\n" "$base" | tee -a "$OUT/summary.txt"
  fi
done
