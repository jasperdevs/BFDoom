...
void bf_emit_code(Inst* inst) {
  Inst* cur = inst;
  int max_pc = 0;
  for (Inst* scan = inst; scan; scan = scan->next) {
    if (scan->pc > max_pc)
      max_pc = scan->pc;
  }
  int max_pc_g = max_pc / 65536;
  ...
  for (int pc_g = 0; pc_g <= max_pc_g; pc_g++) {
    printf("\n# pc_g=%d\n", pc_g);
    ...
    for (int pc_h = 0; pc_h < 256; pc_h++) {
      ...
      for (int pc_l = 0; pc_l < 256; pc_l++) {
        int pc = pc_g * 65536 + pc_h * 256 + pc_l;
        if (!cur)
          break;
        ...
        for (; cur && cur->pc == pc; cur = cur->next) {
          printf("\n# ");
          dump_inst_fp(cur, stdout);
