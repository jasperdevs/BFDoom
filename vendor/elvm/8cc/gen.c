...
static void emit_data_addr(Node *operand, int depth) {
    ...
    Node *base = NULL;
    int v = eval_intexpr(operand, &base);
    if (base == NULL)
        error("internal error in emit_data_addr: %s", node2s(operand));
    Type *ty = base->ty;
    if (base->kind == AST_CONV || base->kind == AST_ADDR)
        base = base->operand;
    if (base->kind == AST_FUNCDESG) {
        emit(".long %s", base->fname);
        return;
    }
    if (base->kind != AST_GVAR)
        error("global variable expected, but got %s", node2s(base));
    assert(ty->ptr);
    int off = v * ty->ptr->size;
    if (off)
        emit(".long %s+%u", base->glabel, off);
    else
...
static void emit_data_primtype(Type *ty, Node *val, int depth) {
    ...
    case KIND_PTR:
        ...
        Node *ptrval = val->kind == AST_CONV ? val->operand : val;
        if (ptrval->kind == AST_LITERAL &&
            ptrval->ty->kind == KIND_ARRAY &&
            ptrval->ty->ptr->kind == KIND_CHAR) {
            emit_data_charptr(ptrval->sval, depth);
            break;
        }
        bool is_char_ptr = ptrval->kind == AST_ADDR &&
                           ptrval->operand->ty->kind == KIND_ARRAY &&
                           ptrval->operand->ty->ptr->kind == KIND_CHAR;
        if (is_char_ptr) {
            emit_data_charptr(ptrval->operand->sval, depth);
        } else if (val->kind == AST_GVAR) {
            emit(".long %s", val->glabel);
        } else if (val->kind == AST_FUNCDESG) {
            emit(".long %s", val->fname);
        } else {
            Node *base = NULL;
